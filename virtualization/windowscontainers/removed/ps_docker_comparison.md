...
* [Docker를 사용하여 Windows 컨테이너 관리](../quick_start/manage_docker.md)
* [PowerShell을 사용하여 Windows 컨테이너 관리](../quick_start/manage_powershell.md) 
...
Hyper-V PowerShell을 사용한 경우 cmdlet의 디자인은 친숙해야 합니다. 워크플로의 대부분은 Hyper-V 모듈을 사용하여 가상 컴퓨터를 관리하는 방법과 비슷합니다. `New-VM`, `Get-VM`, `Start-VM`, `Stop-VM` 대신 `New-Container`, `Get-Container`, `Start-Container`, `Stop-Container`가 있습니다.  몇 가지 컨테이너별 cmdlet 및 매개 변수가 있지만 일반적인 수명 주기 및 Windows 컨테이너의 관리는 Hyper-V VM의 것과 유사하게 보입니다.
...
PowerShell cmdlet은 정확하게 완벽한 패리티가 아니며 PowerShell의 대체로 제공하지 않는 상당히 많은 수의 명령이 있습니다*(특히 `docker build` 및 `docker cp`). 하지만 눈에 들어오는 사실은 `docker run`에 대한 단일 한 줄 대체가 없다는 것입니다.
...
### 사용자 고유의 샘플 빌드
`Get-Command -Module Containers`를 사용하여 모든 컨테이너 cmdlet을 볼 수 있습니다.  여기에서 설명되지 않은 스스로 학습하도록 남겨둔 몇 가지 다른 cmdlet이 있습니다.    
**참고** 코어 PowerShell의 일부인 `Enter-PSSession` 및 `Invoke-Command` cmdlet을 반환하지 않습니다.

`Get-Help [cmdlet name]`, 또는 동등한 `[cmdlet name] -?`을 사용하여 모든 cmdlet에 대한 도움말을 얻을 수 있습니다.  현재 도움말 출력은 자동으로 생성되며 명령에 대한 구문을 알려 줍니다. cmdlet 설계를 완료하는 대로 추가 설명서를 추가할 예정입니다.
...
PS: 완료할 수 있는지 증명하려면 ersatz `docker run`에서 이미 살펴본 cmdlet 중 일부를 구성하는 PowerShell 함수가 여기에 있습니다. (명확히 하자면 이는 현재 개발에 포함되지 않은 개념 증명입니다.)
...
Docker에서 사용 가능한 API 문서를 복제하지 않기 위해 해당 관리 API에 대한 링크를 <a href="https://docs.docker.com/engine/reference/commandline/cli/" >여기</a>에서 제공합니다.  해당 연습은 매우 유용합니다.
