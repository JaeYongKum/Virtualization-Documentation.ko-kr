...
VFP(가상 필터링 플랫폼) 확장은 네트워크 정책을 적용하고 패킷을 조작하는 데 사용되는 확장을 전달하는 Hyper-V 가상 스위치입니다. 예를 들어 VFP는 'overlay' 네트워크 드라이버가 VXLAN 캡슐화를 수행하고 'l2bridge' 드라이버가 수신 및 송신 시 MAC 다시 쓰기를 수행하는 데 사용됩니다. VFP 확장은 Windows Server 2016 및 Windows 10 크리에이터스 업데이트에만 있습니다. 사용자는 다음 두 명령을 실행하여 VFP가 올바르게 실행되고 있는지 확인할 수 있습니다.
...
#### HNS를 사용하려면 컨테이너 호스트 컴퓨터에서 IPv6를 사용하도록 설정 
...
#### Moby Linux VM은 HNS 내부 vSwitch 대신 Windows용 Docker와 함께 DockerNAT 스위치([Docker CE](https://www.docker.com/community-edition) 제품)를 사용합니다. 
...
#### DHCP를 사용하여 가상 컨테이너 호스트에 IP를 할당하려면 MACAddressSpoofing을 사용하도록 설정 
...
#### 고정 IP 할당을 사용하는 경우 *--subnet* 및 *--gateway*를 지정해야 합니다.
...
#### DHCP IP는 L2Bridge 네트워크에서 지원되지 않음
...
#### 기존 vSwitch(Docker에서 보이지 않음)가 투명 네트워크 생성을 차단할 수 있음
