...
<<<<<<< 머리 이유가 어디에 있나요? 건물을 원하는 대로 사용자 지정할 수 있습니다. 여러 종류의 채팅방을 채우거 나, 같은 채팅방에 기입 하거나, 두 가지를 혼합 하 여 사용할 수 있습니다.
=======
![](media/box1.png)
>>>>>>> 원본/마스터
...
<<<<<<< 머리 다음 비디오에서는 Windows 컨테이너에 대해 수행할 수 있는 작업에 대해 자세히 설명 하 고, Microsoft에서 Docker와의 파트너 관계가 열린 원본 컨테이너 개발을 위한 frictionless를 만드는 데 도움이 됩니다. = = = = = = =
![](media/apartment.png)
>>>>>>> 원본/마스터
...
<<<<<<< 머리 Windows 컨테이너를 사용 하 여 작업을 시작할 때 유용한 몇 가지 용어에 대해 알아보겠습니다. = = = = = = =
...
컨테이너의 호스트는 컨테이너용 리소스 집합을 프로비전하고 컨테이너는 이러한 리소스만 사용합니다. 컨테이너는 제공 된 내용 외에 다른 리소스도 존재 하지 않으므로 컨테이너는 인접 한 컨테이너에 대해 프로 비전 된 리소스를 터치 할 수 없습니다.
