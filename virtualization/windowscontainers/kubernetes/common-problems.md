...
사용자는 [CollectLogs](https://github.com/microsoft/SDN/blob/master/Kubernetes/windows/debug/collectlogs.ps1) 스크립트를 실행 하 고 `*portrange.txt` 파일을 컨설팅 하 여이 문제를 식별할 수 있습니다.

또한이 `CollectLogs.ps1`는 `reservedports.txt`에 대 한 성공/실패를 보고 하 고, 임시 TCP 포트 범위에서 포트 풀 할당 가용성을 테스트 하기 위해 HNS 할당 논리를 모방 합니다. 이 스크립트는 10 개의 64 TCP 임시 포트 (HNS 동작 에뮬레이트)를 예약 하 고, 예약 성공 & 오류 수를 계산 하 고, 할당 된 포트 범위를 해제 합니다. 성공 숫자가 10 보다 작은 경우에는 임시 풀에 사용 가능한 공간이 부족 한 것을 나타냅니다. Heuristical에서 64-블록 포트 예약의 수를 대략적으로 요약해 서 보여 줍니다. 또한 `reservedports.txt`에서 생성 됩니다.
...
2. 이 문제를 해결 하기 위해 사용자는 `netsh int ipv4 set dynamicportrange TCP <start_port> <port_count>`같은 명령을 사용 하 여 사용 가능한 임시 포트의 기본 Windows 구성을 늘릴 수도 있습니다. *경고:* 기본 동적 포트 범위를 재정의 하면 임시 범위에서 사용 가능한 TCP 포트를 사용 하는 호스트의 다른 프로세스/서비스에 영향을 미칠 수 있으므로이 범위는 신중 하 게 선택 해야 합니다.
...
이 필드는 Windows CNI 플러그 인에서 영향을 받지 않으므로 Kubernetes `containers.ports.hostPort` 필드를 사용 하 여 포트를 게시할 수 없습니다. 노드에 포트를 게시 하는 시간에 대해 NodePort 게시를 사용 하세요.
...
Windows Server, 버전 1903의 사용자는 다음 레지스트리 위치로 이동 하 여 네트워크 이름 (예: `vxlan0` 또는 `cbr0`)으로 시작 하는 모든 Nic를 삭제할 수 있습니다.
```
...
Windows pod에는 현재 ICMP 프로토콜에 대해 프로그래밍 된 아웃 바운드 규칙이 없습니다. 그러나 TCP/UDP가 지원 됩니다. 클러스터 외부의 리소스에 대 한 연결을 시연 하려는 경우 `ping <IP>`를 해당 `curl <IP>` 명령으로 대체 하세요.
...
이는 Flannel이 올바르게 실행 되지 않았음을 나타냅니다. Flanneld를 다시 시작 하거나 Kubernetes 마스터의 `/run/flannel/subnet.env`에서 수동으로 파일을 복사 하 여 Windows 작업자 노드에 `C:\run\flannel\subnet.env` 하 고 `FLANNEL_SUBNET` 행을 할당 된 서브넷으로 수정할 수 있습니다. 예를 들어 노드 서브넷 10.244.4.1/24가 할당 된 경우:
```
...
이는 Windows에서 현재 네트워킹 스택의 알려진 제한 사항입니다. 그러나 Windows *pod* **는** 서비스 IP에 액세스할 수 있습니다.
...
호스트의 네트워크 어댑터가 "이더넷"이 아닌 경우에는 [InterfaceName](https://github.com/Microsoft/SDN/blob/master/Kubernetes/flannel/l2bridge/start.ps1#L6) 스크립트의 매개 변수를 수정 하는 것이 유용한 경우가 많습니다. 그렇지 않으면 `start-kubelet.ps1` 스크립트의 출력을 참조 하 여 가상 네트워크를 만드는 동안 오류가 발생 하는지 확인 합니다. 
...
### <a name="i-am-still-seeing-problems-what-should-i-do"></a>여전히 문제가 발생 하 고 있습니다. 어떻게 할까요? ### 
...
일시 중지 이미지가 OS 버전과 호환되는지 확인합니다. 이 [지침](./deploying-resources.md) 에서는 OS와 컨테이너 모두 버전 1803 이라고 가정 합니다. 참가자 빌드와 같은 최신 버전의 Windows를 사용하는 경우 그에 따라 이미지를 조정해야 합니다. 이미지에 대해서는 Microsoft의 [Docker 저장소](https://hub.docker.com/u/microsoft/)를 참조하세요. 그럼에 도 불구하고 일시 중지 이미지 Dockerfile과 샘플 서비스 모두 이미지에 `:latest`라는 태그가 지정됩니다.
