...
1. `kube-flannel.yml`의 `net-conf.json` 섹션에서 다음을 두 번 선택 합니다.
 * 클러스터 서브넷 (예: "10.244.0.0/16")은 원하는 대로 설정 됩니다.
 * 백 엔드에서 VNI 4096가 설정 됨
 * 백 엔드에서 포트 4789가 설정 됨
2. `kube-flannel.yml`의 `cni-conf.json` 섹션에서 네트워크 이름을 `"vxlan0"`으로 변경 합니다.

위의 단계를 적용 한 후 `net-conf.json`은 다음과 같이 표시 됩니다.
```json
  net-conf.json: |
    {
...
다음으로, Flannel pod는 Linux 기반 이므로 linux [Nodeselector](https://github.com/Microsoft/SDN/tree/master/Kubernetes/flannel/l2bridge/manifests/node-selector-patch.yml) `kube-flannel-ds` 패치를 대상 linux에만 적용 합니다. (나중에 조인할 때 Windows에서 Flannel "flanneld" 호스트 에이전트 프로세스를 시작 합니다.)

```
kubectl patch ds/kube-flannel-ds-amd64 --patch "$(cat node-selector-patch.yml)" -n=kube-system
```
> [!tip]  
> X86-64 기반이 아닌 노드가 있는 경우 위의 `-amd64`를 프로세서 아키텍처로 바꿉니다.
...
Flannel DaemonSet에는 NodeSelector `beta.kubernetes.io/os=linux` 적용 해야 합니다.
...
> [!tip]  
> 어렵습니까? 다음은 `10.244.0.0/16`기본 클러스터 서브넷에 대해 미리 적용 된 이러한 단계가 있는 Flannel v 0.11.0에 대 한 전체 [예제](https://github.com/Microsoft/SDN/blob/master/Kubernetes/flannel/overlay/manifests/kube-flannel-example.yml) 입니다.
...
1. 사용 중인 네트워크 백 엔드의 유형은 `vxlan`대신 `host-gw`로 설정 됩니다.
2. 클러스터 서브넷 (예: "10.244.0.0/16")은 원하는 대로 설정 됩니다.

2 단계를 적용 한 후 `net-conf.json`은 다음과 같이 표시 됩니다.
```json
net-conf.json: |
    {
      "Network": "10.244.0.0/16",
...
```
> [!tip]  
> X86-64 기반이 아닌 노드가 있는 경우 위의 `-amd64`를 원하는 프로세서 아키텍처로 바꿉니다.
...
> [!tip]  
> 어렵습니까? 다음은 `10.244.0.0/16`기본 클러스터 서브넷에 대해 미리 적용 된 이러한 2 단계가 있는 Flannel v 0.11.0에 대 한 전체 [예제](https://github.com/Microsoft/SDN/blob/master/Kubernetes/flannel/l2bridge/manifests/kube-flannel-example.yml) 입니다.
