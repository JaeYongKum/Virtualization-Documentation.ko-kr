...
|:---------|:---------|:---------|:---------|:---------|
| [Hyper-v 하트 비트 서비스](#hyper-v-heartbeat-service) |  vmicheartbeat | hv_utils | 가상 컴퓨터가 제대로 실행되고 있는지 보고합니다. | 다양 |
...
| [Hyper-V 데이터 교환 서비스(KVP)](#hyper-v-data-exchange-service-kvp) | vmickvpexchange | hv_kvp_daemon | 가상 컴퓨터와 호스트 간에 기본 메타데이터를 교환하는 방법을 제공합니다. | 보통 |
| [Hyper-v 볼륨 섀도 복사본 요청자](#hyper-v-volume-shadow-copy-requestor) | vmicvss | hv_vss_daemon | 볼륨 섀도 복사본 서비스에서 종료하지 않고 가상 컴퓨터를 백업하도록 허용합니다. | 다양 |
...
**설명:** 가상 머신이 운영 체제가 설치되어 있으며 올바르게 부팅되었음을 Hyper-V 호스트에 알립니다.  
**다음에서 추가:** Windows Server 2012, Windows 8  
**영향:** : 사용하지 않도록 설정하면 가상 머신에서 가상 머신 내부의 운영 체제가 제대로 작동하고 있는지 보고할 수 없습니다.  일부 유형의 모니터링 및 호스트 쪽 진단에 영향을 미칠 수 있습니다.  
...
**Linux 디먼 이름:** hv_utils  
**설명:** Hyper-V 호스트에서 가상 머신 종료를 요청할 수 있도록 허용합니다.  호스트는 언제나 가상 컴퓨터를 강제로 끌 수 있지만, 이는 종료를 선택하는 것이 아니라 전원 스위치를 누르는 것과 같습니다.  
**다음에서 추가:** Windows Server 2012, Windows 8  
**영향:** : **강력한** 사용하지 않도록 설정하면 호스트는 가상 머신 내에서 종료를 트리거할 수 없습니다.  모든 종료는 데이터 손실이나 데이터 손상을 일으킬 수 있는 하드 전원 끄기가 됩니다.  
...
**다음에서 추가:** Windows Server 2012, Windows 8  
**영향:** : **강력한** 사용하지 않도록 설정하면 가상 머신의 시계가 이상하게 작동합니다.  
...
**다음에서 추가:** Windows Server 2012, Windows 8  
**영향:** : 사용하지 않도록 설정하면 Windows 8 또는 Windows Server 2012 이전 버전을 실행하는 가상 머신이 Hyper-V 통합 서비스에 대한 업데이트를 수신하지 않습니다.  데이터 교환을 사용하지 않도록 설정하면 일부 유형의 모니터링 및 호스트 쪽 진단에 영향을 미칠 수도 있습니다.  
...
**설명:** 볼륨 섀도 복사본 서비스가 가상 머신에 애플리케이션 및 데이터를 백업할 수 있도록 허용합니다.  
**다음에서 추가:** Windows Server 2012, Windows 8  
**영향:** : 사용하도록 설정하면 가상 머신을 실행하는 중에 백업할 수 없습니다(VSS 사용).  
...
**다음에서 추가:** Windows Server 2012 R2, Windows 8.1  
...
**다음에서 추가:** Windows Server TP3, Windows 10  
...
* [가상 머신에 파일 복사 및 가상 머신의 파일 복사](../user-guide/powershell-direct.md#copy-files-with-new-pssession-and-copy-item)
