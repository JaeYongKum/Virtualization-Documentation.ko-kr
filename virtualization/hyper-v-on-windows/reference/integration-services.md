...
| [Hyper-v 게스트 종료 서비스](#hyper-v-guest-shutdown-service) | vmicshutdown | hv_utils |  호스트에서 가상 컴퓨터 종료를 트리거하도록 허용합니다. | **최고** |
| [Hyper-v 시간 동기화 서비스](#hyper-v-time-synchronization-service) | vmictimesync | hv_utils | 가상 컴퓨터의 시계를 호스트 컴퓨터의 시계와 동기화합니다. | **최고** |
| [Hyper-v 데이터 교환 서비스 (KVP)](#hyper-v-data-exchange-service-kvp) | vmickvpexchange | hv_kvp_daemon | 가상 컴퓨터와 호스트 간에 기본 메타데이터를 교환하는 방법을 제공합니다. | 중간 |
...
**추가됨:** Windows Server 2012, Windows 8  
**영향:** 사용 하지 않도록 설정 하 **는 경우 호스트** 는 가상 머신 내에서 친숙 한 종료를 트리거할 수 없습니다.  모든 종료는 하드 꺼짐 이며이로 인해 데이터 손실이 나 데이터가 손상 될 수 있습니다.  
...
**추가됨:** Windows Server 2012, Windows 8  
**영향:** 사용 하지 않도록 설정 하는 경우에는 가상 머신의 시계가 비정상적 **으로 저하 됩니다** .  
