...
6. "NAT"를 마우스 오른쪽 단추로 클릭하고 "새 인터페이스..."와 "이더넷"을 차례로 선택합니다. 이는 IP가 "10.0.0.4"인 첫 번째 NIC여야 하며 "공용 인터페이스"를 선택하고 인터넷에 연결한 후 이 인터페이스에서 NAT를 사용하도록 설정해야 합니다. 
7. 이제 두 번째 NIC에서 LAN 트래픽을 강제로 내보내려면 고정 경로를 만들어야 합니다. 이 작업은 "IPv4" 아래 "고정 경로" 노드로 이동하여 수행합니다.
