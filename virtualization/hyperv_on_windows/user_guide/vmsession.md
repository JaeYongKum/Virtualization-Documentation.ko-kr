...
> 참고: 세션이 연결되지 않으면 [문제 해결](vmsession.md#troubleshooting)에서 잠재적 원인을 확인하세요. 
...
2. 다음 명령 중 하나를 실행하여 `New-PSSession`으로 가상 컴퓨터에 대한 영구 PowerShell 세션을 만듭니다.
  ...
  > **경고:**  
   14500 이전 빌드에는 버그가 있습니다.  `-Credential` 플래그를 사용하여 자격 증명을 명시적으로 지정하지 않으면 게스트의 서비스가 작동 중단되어 다시 시작해야 합니다.  이 문제가 발생할 경우 [여기](vmsession.md#error-a-remote-session-might-have-ended)에서 해결 지침을 사용할 수 있습니다.
